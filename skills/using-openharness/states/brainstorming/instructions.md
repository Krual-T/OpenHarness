...
- **清晰任务**：改动范围明确、无架构决策、用户的指令可以直接转化为验收标准
- **模糊任务**：问题开放、目标未明确、涉及取舍判断或跨模块影响

**必须**将判断结论及理由向用户陈述，获得用户确认后才分流：
- 用户确认清晰 → 跳至**快通道**
- 用户确认模糊（或推翻 agent 的清晰判断）→ 继续**完整流程**
- 用户未明确确认前，**阻塞**，不得进入任一分支
...
2. **挑战前提**：在提出任何方案之前，先挑战问题本身
   ...
   - 当前矛盾具体是什么？（不能只写"体验不好"）
3. **收集关键信息**：遇到歧义时向用户提问澄清。确认的目的不是消除 agent 的不确定性，而是保证用户和 agent 对问题的理解一致——即使 agent 自认为已经清楚，也可以在步骤 4 后一并确认
4. **提出方案**：方案数量取决于不确定性
   - 不确定性低 → 一个推荐方案 + 至少一个被拒绝的方案及其理由
   - 不确定性高 → 2-3 个方案，含取舍分析和明确推荐
   - 不要因为某个方案看起来明显就跳过替代方案
5. **方案确认停点**：将推荐方案及理由向用户陈述，获得确认后才继续。如果用户否定推荐方案，回到步骤 4 调整
6. **写入 `requirements.md`**：目标用户、核心场景、成功指标、边界、约束、验收标准、至少一个反例。写完后告知用户文件路径，让用户直接在 IDE 中审阅。**不得将全文贴到聊天里**
7. **需求文档确认停点**：用户审阅 `requirements.md` 完毕并确认后，才继续。用户要求修改则回到步骤 6 调整
8. **确认工作流分叉字段**：向下方"工作流分叉字段"章节的三条字段逐一解释含义、给出建议值及理由，**三条字段一并陈述，一次性获得用户确认**后写入 `task-info.yaml`
9. **自检 阶段结束检查**：逐项确认下方"阶段结束检查"章节的 9 个问题都能明确回答
...
- 写入 `requirements.md`
- 确认工作流分叉字段（同上步骤8，三条字段一并陈述、一次性确认）
- 自检阶段结束检查（同上步骤9）

## 工作流分叉字段

阶段结束检查 第 7-9 项依赖以下三条字段，**必须在自检 阶段结束检查 前确认完毕**。三条字段向用户一并解释含义、给出建议值及理由，一次性获得用户确认后写入 `task-info.yaml`。
...
离开需求阶段前，agent 必须逐条自检下面 9 个问题。**自检通过后，将 9 条结论向用户逐条陈述，获得用户确认后才可执行 transition。** 用户否定任一条 → 回到对应步骤修正，重新过 阶段结束检查。
