...
8. **确认工作流分叉字段**：向下方"工作流分叉字段"章节的三条字段逐一解释含义、给出建议值及理由，**必须逐条获得用户确认**后才写入 `task-info.yaml`。不得批量打包确认
...
- 写入 `requirements.md`
- 确认工作流分叉字段（同上步骤8，同样逐条确认）
- 自检阶段结束检查（同上步骤9）

## 工作流分叉字段

阶段结束检查 第 7-9 项依赖以下三条字段，**必须在自检 阶段结束检查 前确认完毕**。每条字段向用户解释含义、给出建议值及理由、询问并获得用户确认后才写入 `task-info.yaml`。
