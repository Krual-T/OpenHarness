...
离开 evidence 收尾前，**必须**能明确回答下面 5 个问题（任何一条答不上来 → 阻塞，不得认为证据已完备）：
...
如果这些问题还答不清，这页就还不是高价值 evidence 索引。**阻塞**，补充证据后再推进。

## Anti-Rationalization

模型会尝试用以下借口跳过证据收尾。这些借口都不成立：

| 借口 | 为什么不成立 |
|------|-------------|
| "文件清单不重要，git log 就能看到" | git log 显示的是所有变更，不是本轮有意为之的关键变更。没有文件清单 = 后续不知道这轮真正改了哪里。 |
| "命令记不记无所谓，反正都能重新跑" | 没有命令记录 = 无法重现验证过程。Final verification command 是唯一的可追溯证据入口。 |
| "这次没有后续事项" | 几乎每个任务都有后续。没写 Follow-ups = 延期事项隐形消失，下次还会踩同一个坑。 |
| "产物路径自己知道就行" | 证据产物只有可被他人找到才有价值。Artifact Paths 必须能让任何协作者定位到关键产物。 |
