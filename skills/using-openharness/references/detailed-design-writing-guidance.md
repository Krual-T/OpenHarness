...
把总体方案收敛成可执行实施计划，先明确改动落点、模块内部职责、接口边界、数据语义、错误处理与迁移顺序，再从这些设计结论推导验证对象、验证路径和预期证据，让人类维护者和 agent 都能按同一组硬约束实施。
...
- 准备怎么验证这轮工作真的成立？
- 如果主验证路径走不通，降级路径是什么？
- `testing-first` 或 verification-first 的实施顺序是什么？
...
- `Runtime Verification Plan`
  - 在实现设计已经具体到足以判断验证对象后，写主验证路径、fallback 和预期证据。
  - 这里必须体现 `testing-first` / verification-first：先准备适合对象的测试或验证，再落实现；它不等于默认先写 pytest。
...
- `Stage Gates`
  ...
  - 至少覆盖实现落点、模块内部职责、接口精度、数据语义、迁移顺序、测试或验证策略、`observability` 要求和预期证据类型。
- `Decision Closure`
...
- `Stage Gates` 至少覆盖实现落点、模块内部职责、数据语义、迁移顺序、测试或验证策略、`observability` 和证据类型。
...
离开 detailed 阶段前，**必须**能明确回答下面 6 个问题（任何一条答不上来 → 阻塞，不得进入下一阶段）：

- 是否已经知道实现会落到哪些文件或模块，以及为什么是这些地方？
- 是否已经知道主要接口边界、接口精度和误用风险？
- 是否已经知道模块内部职责和关键数据语义如何拆分？
- 是否已经知道迁移顺序、切换点和回滚触发点？
- 是否已经知道先准备什么测试或验证，也就是 `testing-first` / verification-first 顺序？
...
- 先把实现设计写到足以判断验证对象，再写验证路径；否则很容易让测试或命令替代真实设计。
