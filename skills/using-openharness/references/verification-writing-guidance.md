...
离开 verification 阶段前，**必须**能明确回答下面 6 个问题（任何一条答不上来 → 阻塞，不得进入下一阶段）：
...
如果这些问题还答不清，**阻塞**。不要把包推进到 `archived`。

## Anti-Rationalization

模型会尝试用以下借口跳过真实验证。这些借口都不成立：

| 借口 | 为什么不成立 |
|------|-------------|
| "测试都通过了，不需要再验证" | 测试通过 ≠ 需求满足。验证要检查的是证据是否真的支撑完成主张，不是测试覆盖率数字。 |
| "上次验证过，这次差不多" | "差不多"不是验证。每次验证必须是 fresh 的——旧结果可能已因代码变更而失效。 |
| "手工跑过一遍了，没问题" | 手工跑 ≠ 可复现验证。手工步骤无法在代码变更时重跑，也不是可追溯证据。 |
| "残留风险很小，不需要写" | 没写下来的风险 = 静默吞掉的盲区。Risk Acceptance 的缺失本身就是最大的风险。 |
