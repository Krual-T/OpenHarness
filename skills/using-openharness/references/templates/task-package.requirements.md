...
**最低要求**：至少写清希望达成的真实结果，不能只包含"提升""优化""增强"这类抽象词。应该能回答"做完以后什么事实会成立"。

## Problem Statement
用中文说明当前痛点、缺口、冲突或风险，解释为什么现在要做这件事。

**最低要求**：至少写清当前矛盾、目标用户、核心场景和为什么现在要做。至少要写出一个已经存在的矛盾，而不是只写未来愿景。
...
**最低要求**：至少写一个 `counterexample`，说明"看起来相关但不属于本轮"的情况。

## Constraints
- 用中文写明协议限制、兼容性约束、依赖条件、`cost cap` 和不可变边界。

**最低要求**：至少写清当前不能突破的协议或兼容性边界，以及本轮 cost cap。如果某个限制被突破就会变成另一个 task package，也写在这里。
