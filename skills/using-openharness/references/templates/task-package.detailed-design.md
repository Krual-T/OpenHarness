...
**最低要求**：至少说明每类改动文件的作用。不接受只有文件清单、没有承载理由的写法。

## Interfaces
用中文写清楚这轮改动暴露或依赖的接口、契约和稳定边界。

**最低要求**：至少定义稳定边界、接口精度（参数语义、输入输出约束、边界条件、错误传播或兼容性要求）和暴露契约。交代关键 observability 入口（日志、状态、测试观察点、验证产物），能说清怎么观察失败或退化。

## Module Internals
用中文说明关键模块的内部职责分解：编排、校验、状态更新、副作用、适配层分别落在哪里。

**最低要求**：至少说明一个关键模块的内部职责分解。这里是把 overview 的结构边界下沉到足够支撑编码的层次，但仍不记录执行结果。

## Data Semantics
用中文说明关键数据结构、字段语义、状态转换或一致性约束。如果关系复杂，说明是否需要用 PlantUML 状态图、类图或关系图辅助表达。

**最低要求**：至少说明一个关键数据结构、字段语义或状态转换约束。

## Stage Gates
用中文写清楚 detailed 要进入实施前必须确定的硬条件。

**最低要求**：至少覆盖实现落点、模块内部职责、接口精度、数据语义、迁移顺序、测试或验证策略、observability 要求和预期证据类型。

## Decision Closure
用中文记录关键挑战如何被处理。只允许写：接受（含理由）、拒绝（含理由、替代方案）、延期（含触发条件）。

**最低要求**：至少记录一个被接受、拒绝或延期的关键挑战，并带各自理由。

## Error Handling
用中文说明失败路径、误用风险、校验边界和如何避免静默出错。

**最低要求**：至少写出一个静默出错风险，并交代异常如何传播或回退。如果异常处理跨模块传播，应写清传播链。

## Migration Notes
用中文描述迁移顺序、兼容策略、落地阶段和回滚注意事项，说明切换点与回滚触发点。

**最低要求**：至少写出实施顺序、切换点和回滚触发点。没有迁移笔记 = 默认"改完就生效"，生产环境从没有这种事。

## Recommended Diagrams
如果关键交互、状态变化或数据关系仅靠文字容易歧义，用中文说明推荐补哪些 PlantUML 图（时序图、状态图、数据关系图等）。

**最低要求**：如果图示能显著减少歧义，应给出推荐图类型或图示位置。图不能替代文字里的接口、数据语义和异常说明。

## Detailed Reflection
用中文记录对测试策略、接口边界、迁移假设和验证路径的反思。

**最低要求**：至少再次挑战测试策略、接口、迁移和预期证据假设。
