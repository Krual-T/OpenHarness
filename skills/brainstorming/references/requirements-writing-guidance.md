...
离开需求阶段前，**必须**能明确回答下面 7 个问题（任何一条答不上来 → 阻塞，不得进入下一阶段）：
...
- task_type（`mechanical` / `standard development` / `protocol/architecture`）是否已确认并写入 `STATUS.yaml`？

如果这 7 个问题还答不上来，**阻塞**。不要进入 `02-overview-design.md`。

## Anti-Rationalization

模型会尝试用以下借口跳过需求收敛。这些借口都不成立：

| 借口 | 为什么不成立 |
|------|-------------|
| "需求已经很清楚了，直接开始吧" | 写下来才能暴露隐藏歧义。"清楚"是主观感觉，不是可验证事实。 |
| "用户说了要什么，不需要再分析" | 用户说的 ≠ 需求。需求需要转化为可验证的验收标准，否则验证阶段无法判断完成。 |
| "先写代码，需求后面补" | 后面不会补的。代码写完再补需求 = 让需求服从实现，而不是实现服务于需求。 |
| "这次改动小，不需要完整需求分析" | 改动大小与需求歧义无关。小改动也可能有模糊边界，边界模糊就会在实现时引发争议。
