...
- `no-harness bootstrap workflow`
  - define the first round entry sequence for repos with no design packages and no reusable harness structure
  - define which files or artifacts OpenHarness should scaffold immediately versus leave to later focused tasks
  - likely touch `AGENTS.md`, `README.md`, `using-openharness`, templates, and bootstrap-oriented references
