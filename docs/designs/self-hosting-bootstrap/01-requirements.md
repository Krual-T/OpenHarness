...
- there is no `docs/designs/` package tree
- the Python-based harness CLI cannot run in a clean checkout because `PyYAML` is undeclared
- the repo tests still target legacy layout assumptions and historical design-package fixtures from another repository
...
## Constraints
- Keep the existing fixed `docs/designs/<task>/` package model for this round.
