...
- Verification Path:
  - Run `uv run python skills/using-openharness/scripts/openharness.py check-tasks` after protocol docs, templates, and compatibility package wording are updated.
...
- The repository may continue to store task packages under `docs/task-packages/` until a later dedicated path-migration round proves worthwhile.
