# OH-011 Task Package Completion Semantics

## Summary
- This archived package recorded the migration that renamed the repository's core end-to-end work unit from `design package` to `task package`.
- Repair the archive contract so `archived` no longer gets used for packages whose design is done but implementation still remains.

## Current Status
- Archived.
- The task-package naming and archive-semantics repair is implemented, verified, and kept as the historical baseline for future protocol cleanup.

## Read This First
- `STATUS.yaml`
- `01-requirements.md`
- `02-overview-design.md`
- `03-detailed-design.md`
- `05-verification.md`
- `06-evidence.md`
