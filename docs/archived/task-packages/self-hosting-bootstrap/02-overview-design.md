...
- Scaffold `docs/archived/task-packages/self-hosting-bootstrap/` using the existing harness templates and use it as the first active package.
...
3. The CLI discovers `docs/archived/task-packages/self-hosting-bootstrap/` as the active package.
4. `check-tasks` validates the package and `pytest` validates the self-hosting assumptions.
