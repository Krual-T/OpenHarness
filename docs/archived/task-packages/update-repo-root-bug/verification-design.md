...
## 验证路径
- **计划路径**：新增单元测试，用 `CliRunner` 调用 `openharness --repo <tmp_repo> update`，monkeypatch `subprocess.run` 拦截真实命令，断言 `git pull` 和 `uv tool upgrade --reinstall openharness` 的 `cwd` 都等于 `<tmp_repo>`。
...
| 命令 | 期望退出码 | 期望输出 |
|------|------------|----------|
| `uv run pytest tests/openharness_cases/test_cli_workflows.py -k update_uses_harness_context_repo_root` | 0 | 新增 update cwd 回归测试通过 |
...
- `update` 使用 `HarnessContext.current().config.repo_root`。
- 测试证明 `--repo` 指定目录会传入 update 子进程的 `cwd`。

## 可追溯性
| 需求结果 | 验证方法 | 证据位置 |
|----------|----------|----------|
| 使用 `HarnessContext.current().config.repo_root` | 代码审查和新增测试 | `openharness_cli/commands/update.py`、测试输出 |
| `git` 和 `uv` 子进程 cwd 来自 repo root | monkeypatch `subprocess.run` 记录调用 | `tests/openharness_cases/test_cli_workflows.py` |
