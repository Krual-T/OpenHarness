...
- Planned Evidence:
  ...
  - removal of `skills/writing-skills/` and `skills/executing-plans/`
...
- Tests should assert boundary rules that matter to routing, such as `openharness` remaining the only repository entry skill and compatibility helpers not presenting themselves as fixed protocol.
...
- `writing-plans` now becomes a narrower compatibility stub that points only to `subagent-driven-development`; it should stop advertising inline execution through `executing-plans`.
...
- I checked whether deleting `writing-skills` is safe even though it is an imported generic skill rather than a compatibility helper. It is safe because the user explicitly requested removal, and no core repository protocol depends on it.
