...
- `skills/using-openharness/states/planning/instructions.md` — 明确验证设计阶段消费并校验需求阶段确定的 `verify_by`，补充文档字符级断言边界和 `qualitative` 审核交接包要求。
- `skills/using-openharness/states/verifying/instructions.md` — 补充定性审核交接包执行、子 Agent 逐项审核、人工逐项反馈、分歧处理和证据完整性要求。
- `skills/using-openharness/references/templates/task-package.plan.md` — 增加 `## 审核交接包` 和 `### 审核矩阵` 模板结构。
...
- `docs/task-packages/document-verification-and-qualitative-handoff/plan.md` — 记录验证计划和审核交接包。
...
用户指出 `planning/instructions.md` 中 `## verify_by 选择约束` 仍会误导阶段职责，且文档字符级断言边界不应对所有 `verify_by` 暴露。已增量修正为：
...
- `planning/instructions.md`：将 `verify_by` 冲突回退动作统一为回退需求阶段，修正 `requirements.md` 和 `task-info.yaml` 后再进入验证设计。
...
`verifying` 阶段按 `plan.md` 的审核交接包执行。

### 实施阶段自查

| 审核对象 | 自查项 | 结果 |
|----------|--------|------|
| `planning/instructions.md` | 是否仍暗示本阶段可静默重新选择 `verify_by` | 未发现静默切换要求；新增说明要求冲突时回退需求阶段 |
| `planning/instructions.md` | 是否仍以“选择约束”展示全量 verify_by 分流规则 | 已改为 `## verify_by 一致性校验`，并按 Jinja 条件渲染当前类型 |
| `planning/instructions.md` | 是否说明字符级断言适用和不适用边界 | 已新增对应章节 |
| `verifying/instructions.md` | 是否要求完整交接包和人工逐项反馈 | 已新增对应流程和失败处理 |
| `task-package.plan.md` | 是否有审核交接包和审核矩阵位置 | 已新增稳定章节 |
