...
- 每次提交前都要修改pyproject.toml中的project.version的版本号
   - 使用x.y.z 版本号（通常称为语义化版本，Semantic Versioning）
   - x（主版本号，Major）：当做了不兼容的 API 修改时递增。这意味着新版本可能无法直接兼容旧项目，用户需要修改代码或配置才能升级。例如，从 1.x.x 到 2.0.0。
   - y（次版本号，Minor）：当以向后兼容的方式添加了功能时递增。通常是新增了某个功能或特性，但不会破坏原有功能，用户可以放心升级。例如，从 1.1.0 到 1.2.0。
   - z（修订号，Patch）：当做了向后兼容的问题修正时递增。主要用于修复 Bug、安全漏洞或性能问题，不涉及新功能或 API 变更。例如，从 1.1.1 到 1.1.2。
